...
async def brackets_counter(message: Message):
    ...
    open_bracket_count = message.text.count('(')
    close_bracket_count = message.text.count(')')
    if open_bracket_count >= close_bracket_count:
        return
    current_value = close_bracket_count - open_bracket_count
    ...
    if stat[1] + current_value <= 10:
        ...
        await message.answer(f'Вы превысили лимит ногтей! Сообщение было удалено!')
